...
class Name:
  def __init__(self, parent):
    self.quiz_frame = Frame(window, bg="purple")
    self.quiz_frame.pack(fill="both", expand=1)
    self.heading_label = Label(window, text = "Intelligence Quiz", font = ("Tw Cen", "18", "bold"), bg = "purple")
    self.heading_label.place(x=468, y=100)
    self.user_label = Label (window, text = "Please enter your name below",  font = ("Tw Cen", "16"), bg = "purple")
    self.user_label.grid(row = 3, padx= 70, pady = 20)
	 
    self.entry_box = Entry(window)
    self.entry_box.grid(row = 4, pady = 20)

    self.continue_button = Button (window, text = "Continue", bg = "darkseagreen1", command=self.name_collection)
  ...
  def name_collection(self):
      ...
      if name == '':
        ...
        messagebox.showerror('Name error','Name can only consist of letters')        
      elif len(name) <1:
...
#Question label that is configured to say different questions in the dictionary keys.
    self.question_label = Label(self.quiz_frame, text = "Q: " + self.question_bank[qnum][0], font = ("Helvitica","16", "bold"), foreground = 'white', bg = '#FFD700', highlightbackground = 'white', pady = 5, width = 52, highlightthickness = 2, wraplength = 700) #Attributes.
    ...
    self.exit_button.place(x = 20, y = 530) #Widget Placement.
      ...
      #Next button.
    self.next_button = Button(self.quiz_frame, text = "NEXT", font = ("Helvetica", "14", 'bold'), foreground = 'white', bg = '#D9EAD3', pady = 10, width = 10, highlightthickness = 2, highlightbackground = 'black',  activebackground = '#649568', command = self.endresults) #Attributes.
  ...
  def question_change(self):
      ...
      self.question_label.config(text = "Q: " + self.question_bank[qnum][0]) #Configure question label to be the question in a key from dictionary.
      self.option1.config(text = self.question_bank[qnum][1]) #Radio button options become the answers from a key in dictionary.
      self.option2.config(text = self.question_bank[qnum][2])
      self.option3.config(text = self.question_bank[qnum][3])
      self.option4.config(text = self.question_bank[qnum][4])

  #Score calculations.
  def score_bank(self):
      global score #Instance labels of classes are equal to a variable name so I can configure the texts easily.
      ...
      if len(asked)>9: #If the last question (10th) is asked.
        if option_choice == self.question_bank[qnum][6]: #If last question is right answer.
          ...
          answer_text.configure(text = "Incorrect: \n" + self.question_bank[qnum][5], foreground = 'red') #Configure answer text to say user is incorrect and why.
          
      else:
        if option_choice == 0: #Check if user made a choice.
          answer_text.configure(text = "Sorry you didn't select anything, please retry", foreground = 'red')
        else: #If they made choice that isn't last question.
         if option_choice == self.question_bank[qnum][6]: #If user is right.
            self.score += 1
            self.question_number += 1
            total_score.configure(text = self.score)
            question_counter.configure(text = self.question_number, foreground = 'black')
            ...
            total_score.configure(text = self.score)
            question_counter.configure(text = self.question_number, foreground = 'black')
            answer_text.configure(text = "Incorrect: \n" + self.question_bank[qnum][5], foreground = 'red')

  def endresults(self):
        window.destroy()
        open_end_object = End()

class End:  # Class for exit box and the exit button

    def __init__(self):
      global window2# Constructor, The __init__() function is called automatically every time the class is being used to create a new object
        #Setting up the frame.
      self.results_frame = Frame(window2)
      window2.geometry("1050x600") #Geometry used to create a fixed window size/window dimensions.
     

      #Background image of window.
      self.bg_image = Image.open("5.png") 
      self.bg_image = ImageTk.PhotoImage(self.bg_image)
      window2.configure(bg = background_color) 
      self.bg_label = Label(self.results_frame, image = self.bg_image)
      self.bg_label.place(x = 0, y = 0) # make label fit the parent window always

    # Method to destroy end screen box after pressing exit button
